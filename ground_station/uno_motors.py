...
# LOWERED 0.15 -> 0.60 ON 2026-08-24, on the operator's report that the wheels
# still cut out on a fast stick move: "when its instant move to power is cut
# off". That is the brown-out this limiter exists to blunt, and 150 ms was not
# blunting it enough - the warning above said as much and it turned out to be
# right. 600 ms spreads the same current step over ~30 send frames instead of 7.
#
# THIS IS A TREATMENT, NOT A CURE, and the note above still stands: the driver
# rail browns out under a current step that a healthy supply would absorb. A
# longer ramp hides it by never presenting the step. Fix the supply and this can
# go back down; until then, do not raise it to chase responsiveness.
# DISABLED 2026-08-24 on the operator's instruction: "remove slow to fast
# acceleration, only make it start in 35% joystick". The wheels now follow the
# stick with no ramp at all - past the dead band the demand is whatever the
# stick says, on the same frame.
#
# READ THIS BEFORE PUTTING A RAMP BACK OR TAKING IT FURTHER. This limiter was
# the only thing blunting the current STEP into a driver rail that is already
# browning out, and the operator reported that brown-out himself earlier the
# same day ("when its instant move to power is cut off"). Removing the ramp does
# not fix that; it removes the thing that was hiding it. If the wheels cut out
# on a fast stick move, this is why, and the fix is the supply - not this line.
#
# Restore instantly without editing: MOTOR_SLEW_PER_S=1700 gives the old 150 ms
# ramp, 425 gives the 600 ms one.
# RAMP BACK ON, 2026-08-24, at 400 ms - the value this file carried before
# 2026-08-19, not the 150 ms that replaced it.
#
# WHY 400 AND NOT 150. The operator has now measured the fault this limiter
# exists for: at full throttle the motor rail sags to 8 V. 150 ms was the
# setting in place when the wheels were reported cutting out, so restoring it
# would restore the symptom. 400 ms spreads the same current step over ~20 send
# frames instead of ~7.
#
# IT IS MADE WORSE BY THE FIRMWARE, and that part is not fixable from here:
# uno_eth_link.ino has MIN_DUTY = 90, so the FIRST moving demand is already 35%
# duty. This ramp now has to carry a step that starts at a third of full power
# rather than at zero. Lowering MIN_DUTY needs the Uno on USB and a reflash.
#
# STILL A TREATMENT, NOT A CURE. Nothing measures that rail - there is no
# voltage or current sense anywhere on this rig - and a 12 V supply reaching
# 8 V under load is a battery, wiring or connector fault. This only stops the
# software from presenting the step that exposes it.
MOTOR_SLEW_PER_S = float(os.environ.get("MOTOR_SLEW_PER_S", str(MAX_PWM / 0.40)))

# DECELERATION, which used to be unlimited. Operator asked for the wheels to
# ease off as well as ease on ("acceleration and deacceleration, not instant").
#
# SEPARATE FROM, AND FASTER THAN, THE ACCELERATION RATE - 250 ms against 600 ms
# - because the two are not the same risk. Ramping UP protects the supply.
# Ramping DOWN costs stopping distance, and slew()'s own docstring is blunt
# about why that was unlimited: "a limiter that can slow down a STOP is a
# limiter that can run the robot into something". So the deceleration limit is
# deliberately gentle enough to feel smooth and short enough that a released
# stick still stops the rig in a quarter second.
#
# THE FAILSAFE PATH IGNORES IT ENTIRELY. A stale reader, a dead ADC or a dropped
# packet still zeroes both wheels in one frame - see the `fresh` branch in the
# send loop. What is ramped here is only an OPERATOR asking for less power, not
# the machine deciding to stop.
#
# IT DOES NOT APPLY TO A STOP. slew() returns 0 immediately for a zero target,
# so this only ever ramps between two MOVING speeds - full to half, say. Coming
# off the throttle altogether is as instant as it ever was.
#
# Set to 0 to restore the old unlimited deceleration.
# OFF with the acceleration ramp - see above. Easing off is instant again,
# which it was before 2026-08-24 anyway. MOTOR_DECEL_PER_S=1020 restores the
# 250 ms ease-off if it is ever wanted on its own.
MOTOR_DECEL_PER_S = float(os.environ.get("MOTOR_DECEL_PER_S", "0"))
...
def slew(prev, target, max_step, down_step=0.0):
    """Rate-limit a RISE in demand, and optionally a FALL.

    `down_step` of 0 keeps the original behaviour exactly: falling and stopping
    are immediate. Anything above 0 limits deceleration too - see
    MOTOR_DECEL_PER_S for why that is a separate, larger number and why the
    failsafe path must not use it.
    ...
    # A STOP IS NEVER RAMPED, whoever asked for it. Operator's call 2026-08-24:
    # easing between two moving speeds should be smooth, but zero means stop and
    # stop means now.
    #
    # This is also what puts slew()'s original safety property back after the
    # deceleration limit went in. That limit is for a hand easing off the
    # throttle; it must never stand between "stop" and stopped, whether the zero
    # came from a released stick, a centred axis inside the dead band, or the
    # failsafe. One test covers all three, which is why it is here and not in
    # the caller.
    if target == 0:
        return 0
    ...
    if not reversing and abs(target) <= abs(prev):
        ...
        # Easing off under the deceleration limit. Toward zero from either
        # sign, so the step is applied to the MAGNITUDE and the sign is put
        # back - stepping the signed value would accelerate a negative demand.
        eased = max(abs(target), abs(prev) - down_step)
        return int(eased if prev >= 0 else -eased)
        ...
        try:
            while not self._stop.is_set():
                ...
                try:
                    ...
                    self._slew_at = now
                    span = max(0.0, min(dt, 0.5))
                    max_step = MOTOR_SLEW_PER_S * span
                    # A STALE READER IS NOT A DECELERATION - it is a failsafe,
                    # and it stops the wheels in one frame as it always has. The
                    # ramp below is only for an operator easing off.
                    down_step = MOTOR_DECEL_PER_S * span if fresh else 0.0
                    left = slew(self._prev_left, left, max_step, down_step)
                    right = slew(self._prev_right, right, max_step, down_step)
